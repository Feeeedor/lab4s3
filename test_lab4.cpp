...
TEST(old_student, constructor_empty)
{
    ...
    ASSERT_EQ(s.getNumber_score(), 0);
    ASSERT_EQ(s.getScore().empty(), true);
    ASSERT_EQ(s.getYir(), "no_data");
    ASSERT_EQ(s.getPlace_yir(), "no_data");
...
TEST(old_student_student, set_yir)
{
    ...
    ASSERT_EQ(stud->getFamily(), "no_data");
    ...
    ASSERT_EQ(dynamic_cast<Old_student *>(stud)->getPlace_yir(), "no_data");
...
{
    Group g;
    ASSERT_EQ(g.getType_students(), "no_data");
    ASSERT_EQ(g.getIndex(), "no_data");
...
TEST(Group_table, insert)
{
    ...
    g2->setIndex(ind);
    g.insertGroup(g2);
    ASSERT_EQ(g.begin()->second->getIndex(), "jun");
    ASSERT_EQ(g.begin()->second->getNumber_disciplines(), 0);
    ASSERT_EQ(g.begin()->second->getType_students(), "no_data");
